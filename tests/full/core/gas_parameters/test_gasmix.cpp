...
static std::unique_ptr<const_parameters> methane(
    const_parameters::Init(GAS_TYPE_METHANE,
                           0.00617,
                           4226000,
                           190.66,
                           0.0,
                           16.043,
                           0.011));
static std::unique_ptr<const_parameters> ethane(
    const_parameters::Init(GAS_TYPE_ETHANE,
                           0.0049,
                           4871000,
                           305.33,
                           0.0,
                           30.07,
                           0.105));
static std::unique_ptr<const_parameters> n_butane(
    const_parameters::Init(GAS_TYPE_N_BUTANE,
                           0.0 /*0.00438*/,
                           3800000,
                           425.66,
                           0.274,
...
// Vk setup
static std::unique_ptr<const_parameters> toluene(
    const_parameters::Init(GAS_TYPE_TOLUENE,
                           0.316 / 92.14,
                           4130000,
                           593.0,
                           0.284,
                           92.14,
                           0.266));
static std::unique_ptr<const_parameters> n_hexane(
    const_parameters::Init(GAS_TYPE_HEXANE,
                           0.37 / 86.178,
                           3030000,
                           507.85,
                           0.263,
...
class MixtureCriticalTest : public ::testing::Test {
 protected:
  ...
  void set_tk_test_case() {
    pm = parameters_mix{{0.193, {*methane, dyn_parameters()}},
  ...
  void set_vk_test_case() {
    pm = parameters_mix{{0.495, {*toluene, dyn_parameters()}},
...
TEST(component_InitTest, MethaneInit) {
  ...
  fs::path methane_path =
      gas_paths::cwd / gas_paths::xml_path / gas_paths::xml_methane;
...
TEST(component_InitTest, PropaneInit) {
  fs::path propane_path =
      gas_paths::cwd / gas_paths::xml_path / gas_paths::xml_propane;
...
TEST(mixture_InitTest, RK2MixtureFileInit) {
  fs::path gasmix_path =
      gas_paths::cwd / gas_paths::xml_path / gas_paths::xml_gasmix;
  ASSERT_TRUE(fs::exists(gasmix_path));
  std::unique_ptr<GasMixComponentsFile<XMLReader>> gasmix_comps(
      GasMixComponentsFile<XMLReader>::Init(rg_model_t::REDLICH_KWONG, nullptr,
                                            gasmix_path.string()));
...
TEST(mixture_InitTest, DISABLED_GOSTMixtureFileInit) {
  ...
  ASSERT_TRUE(fs::exists(gasmix_path));
  std::unique_ptr<GasMixComponentsFile<XMLReader>> gasmix_comps(
      GasMixComponentsFile<XMLReader>::Init(rg_model_t::NG_GOST, nullptr,
                                            gasmix_path.string()));
  ASSERT_TRUE(gasmix_comps != nullptr);
  EXPECT_TRUE(gasmix_comps->GetMixParameters() != nullptr);
}
