...
- Рид Р., Праусниц Дж., Шервуд Т. "Свойства газов и жидклстей: Справочное пособие". Л.:Химия, 1982.
- Павловский В.А. "Введение в термодинамику реальныхгазов". _Тут данные издания_.
...
- *опционально* [gtest](https://github.com/google/googletest) - Google Test, C++ тест фрэймворк от Google
...
Итак, на март 2020 cmake-файл подключения тестов `tests_run.cmake` добавлен в .gitignore, но если что, его содержимое:    
<code>add\_subdirectory(tests/full)</code>    
В указанной выше директории находится проект гуглотестов. Проект специализирован CMakeLists.txt файлом, но в нём заданы переменные GTEST\_ROOT и GTEST\_INCLUDE\_DIR, которые специфичны для моих ПК ¯\\\_(ツ)\_/¯.
