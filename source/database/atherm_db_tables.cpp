...
static const db_table_create_setup model_info_create_setup(
    table_model_info, model_info_fields, mi_uniques, nullptr);
...
static const std::shared_ptr<db_ref_collection> calculation_info_references(
   ...
   db_reference(TABLE_FIELD_NAME(CI_MODEL_INFO_ID), table_model_info,
      TABLE_FIELD_NAME(MI_MODEL_ID), true)});
static const db_table_create_setup calculation_info_create_setup(
    table_calculation_info, calculation_info_fields, ci_uniques,
...
static const std::shared_ptr<db_ref_collection> calculation_state_log_references =
  ...
  new db_ref_collection {
  db_reference(TABLE_FIELD_NAME(CSL_INFO_ID), table_calculation_info,
...
static const db_table_create_setup calculation_state_log_create_setup(
    table_calculation_state_log, calculation_state_log_fields, sci_uniques,
...
const db_fields_collection *AthermDBTables::GetFieldsCollection(db_table dt) const {
...
const db_table_create_setup &AthermDBTables::CreateSetupByCode(db_table dt) const {
  switch (dt) {
    case table_model_info:
...
template <>
void IDBTables::setInsertValues<calculation_info>(db_query_insert_setup *src,
...
template <>
void IDBTables::SetSelectData<calculation_info>(db_query_select_result *src,
    std::vector<calculation_info> *out_vec) const {
  for (auto &row: src->values_vec) {
    calculation_info ci;
    for (auto &col: row) {
      if (src->isFieldName(TABLE_FIELD_NAME(CI_CALCULATION_ID), src->fields[col.first])) {
        ci.id = std::atoi(col.second.c_str());
        ci.initialized |= calculation_info::f_calculation_info_id;
      } else if (src->isFieldName(TABLE_FIELD_NAME(CI_MODEL_INFO_ID), src->fields[col.first])) {
        ci.model_id = std::atoi(col.second.c_str());
        ci.initialized |= calculation_info::f_model_id;
      } else if (src->isFieldName(TABLE_FIELD_NAME(CI_DATE), src->fields[col.first])) {
