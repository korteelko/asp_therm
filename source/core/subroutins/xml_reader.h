...
class gasxml_node {
 ...
 public:
  gasxml_node(xml_node_t node) : node(node) {}

  const gasxml_node<xml_node_t>* ChildByName(const std::string& name) const {
    ...
    if (first_child != nullptr) {
      if (first_child->GetName() == name) {
        ...
        Logging::Append(
            ...
            "Search for " + name + " but get %s" + first_child->GetName());
#endif  // READERS_TEST
  ...
  void SetName(const std::string& name) { node.name = name; }

  void SetValue(const std::string& value) { node.value = value; }
};
...
class XMLReader {
 public:
  static XMLReader<node_t>* Init(std::string gas_xml_file) {
    ...
    if (err == ERROR_SUCCESS_T) {
      if (!is_exists(gas_xml_file)) {
        ...
        Logging::Append(
            err, std::string(std::string("cannot open file: ") + gas_xml_file)
                     .c_str());
      } else {
  ...
  std::string GetFileName() const { return gas_xml_file_; }

  merror_t GetErrorCode() const { return error_.GetErrorCode(); }

  /* todo: remove(replace with LogError()) */
  std::string GetErrorMessage() const { return error_.GetMessage(); }

  /* todo: remove it */
  std::string GetRootName() const { return gas_root_node_->GetName(); }
 ...
 private:
  XMLReader(std::string gas_xml_file)
    ...
    xml_root_node_ = std::unique_ptr<pugi::xml_node>(new pugi::xml_node());
    ...
    *xml_root_node_ = xml_doc_.child(root_node_name.c_str());
    gas_root_node_ = std::unique_ptr<gasxml_node<node_t>>(
        new gasxml_node<node_t>(node_t(NODE_T_ROOT, root_node_name.c_str())));
   ...
   */
  void tree_traversal(pugi::xml_node* xml_nd, gasxml_node<node_t>* gasxml_nd) {
   ...
   * \param gasxml_nd - соответствующая нода в gas_tree(class XMLReader)
   */
  void init_subnode(pugi::xml_node* pugi_nd, gasxml_node<node_t>* gasxml_nd) {
   ...
   * \param gasxml_nd - соответствующая нода в gas_tree(class XMLReader)
   */
  void init_leafnode(pugi::xml_node*& xml_nd, gasxml_node<node_t>* gasxml_nd) {
