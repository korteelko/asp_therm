...
public:
  static ComponentByFile *Init(const std::string &filename) {
    ConfigReader<gas_node> *config_doc =
        ConfigReader<gas_node>::Init(filename);
    ...
    if (config_doc_) {
      ...
      if (initCriticalPoint(tmp_vec, cp)) {
        ...
        if (!set_double(get_val(tmp_vec, "molec_mass"), &mol) &&
          ...
          set_gas_name();
          result.reset(const_parameters::Init(
  ...
  std::shared_ptr<dyn_parameters> GetDynParameters() {
    ...
    if (config_doc_) {
      ...
      if (initPoint(tmp_vec, pnt)) {
        ...
        double dyn[DYN_PARAMS_COUNT];
        dyn_setup ds = initDynSetup(tmp_vec, dyn);
        result.reset(dyn_parameters::Init(
            ds, dyn[0], dyn[1], dyn[2], parameters {pnt[0], pnt[1], pnt[2]}));
      }
    }
    return result;
  }
