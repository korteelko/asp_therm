...
calculation_info::calculation_info(std::time_t dt) : datetime(dt) {
...
mstatus_t calculation_info::SetTime(const std::string& time) {
  ...
  if (!time.empty()) {
    ...
    if (i == 2) {
      ...
      std::time_t res_time = mktime(as_tm);
      bool NotOverride =
          (as_tm->tm_hour == time_arr[0]) && (as_tm->tm_min == time_arr[1]);
...
std::string calculation_info::GetDate() const {
  ...
  snprintf(d, sizeof(d) - 1, "%04d/%02d/%02d", as_tm->tm_year + 1900,
...
std::string calculation_info::GetTime() const {
  char t[16] = {0};
  std::tm* as_tm = std::localtime(&datetime);
  snprintf(t, sizeof(t) - 1, "%02d:%02d", as_tm->tm_hour, as_tm->tm_min);
  return std::string(t);
}
