...
model_str::model_str(rg_model_id mn,
                     int32_t vmaj,
                     int32_t vmin,
                     const std::string& info)
    : model_type(mn), vers_major(vmaj), vers_minor(vmin), short_info(info) {}

std::string model_str::GetString() const {
  return "\nРасчётная модель:\n" + short_info
         + "\nКод: " + std::to_string((uint64_t)model_type.type) + "."
         + std::to_string(model_type.subtype) + "\nВерсия: "
         + std::to_string(vers_major) + "." + std::to_string(vers_minor);
}

namespace update_configuration_functional {
typedef std::function<merror_t(program_configuration*,
                               const std::string& value)>
    update_models_config_f;
...
merror_t update_rk_soave_mod(program_configuration* mc,
                             const std::string& val) {
  return (mc) ? set_bool(val, &mc->calc_cfg.rk_enable_soave_mod)
              : ERROR_INIT_ZERO_ST;
}
merror_t update_rk_orig_mod(program_configuration* mc, const std::string& val) {
  return (mc) ? set_bool(val, &mc->calc_cfg.rk_enable_origin_mod)
              : ERROR_INIT_ZERO_ST;
}
merror_t update_pr_binary_coefs(program_configuration* mc,
                                const std::string& val) {
  return (mc) ? set_bool(val, &mc->calc_cfg.pr_enable_by_binary_coefs)
              : ERROR_INIT_ZERO_ST;
}
merror_t update_enable_iso_20765(program_configuration* mc,
                                 const std::string& val) {
  return (mc) ? set_bool(val, &mc->calc_cfg.enable_iso_20765)
              : ERROR_INIT_ZERO_ST;
}
